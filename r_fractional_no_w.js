// r дробные, z отстутствуют, из s отнимается все соответствующее t, нет w
...
function build_model(dict){
    ...
    get_s_constraints(constraints, t_car,"car",s)
    get_s_constraints(constraints, t_car, "rail",s)
    get_s_constraints(constraints, t_car, "plane",s)
...
function get_s_constraints(constraints,arr,str,s){
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        if(t_time!=0){
            constraints[`s${i+1}_${str}`] = {"max":s-t_time}
        }
    }
}
...
function get_r_variables_ints(variables, ints, c_arr, gk_arr, t_arr, str){
    ...
    for(let i = 0; i < c_arr.length; i++){
        for(let j = 0; j < c_arr[i].length; j++){
            if(c_arr[i][j]!=0){
                //дробные r и нет z. из s отнимается все соответствующее t
                temp_v = new Object()
                gk = gk_arr[i][j]
